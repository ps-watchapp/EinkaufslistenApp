...
import de.uni_muenster.wi.md2library.controller.eventhandler.implementation.Md2OnAttributeChangedHandler;
			...
			return  
			new 
			Md2String(((__ReturnStepStack)content).getReturnStep());	
			} else { return null;}
			case "returnAndReverseStep":
			if(((__ReturnStepStack)content).getReturnAndReverseStep() != null){
			return  
			new 
			Md2String(((__ReturnStepStack)content).getReturnAndReverseStep());	
			} else { return null;}
			case "returnAndProceedStep":
			if(((__ReturnStepStack)content).getReturnAndProceedStep() != null){
			return  
			new 
			Md2String(((__ReturnStepStack)content).getReturnAndProceedStep());	
			} else { return null;}
			case "tail":
