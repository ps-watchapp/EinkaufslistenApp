...
public class ArtikelAnsehen___ArtikelAnsehen_processChainExecuteStepAction_Action extends AbstractMd2Action {

    public ArtikelAnsehen___ArtikelAnsehen_processChainExecuteStepAction_Action() {
		super("ArtikelAnsehen___ArtikelAnsehen_processChainExecuteStepAction_Action"); 
	}
    ...
    public void execute() {
		...
		if((Md2ContentProviderRegistry.getInstance().getContentProvider("__processChainControllerStateProvider").getValue("currentProcessChainStep").equals(new Md2String("firstProcessChain__firstStep")))){
			...
			try {
				var1 = new Md2CallTask(new Md2GoToViewAction(EinkaufszettelApp.getAppContext().getString(R.string.ArtikelAnsehenViewActivity)));
				var1.execute();
